 ...
 * @date 10/08/2018
 */
class InputStreamMessageReader(inputStream: InputStream):MessageReader {

    val inputStream: InputStreamIterator

    init {
        this.inputStream = if (inputStream is InputStreamIterator) inputStream else InputStreamIterator(inputStream)
    }

    companion object {
        private val log = LoggerFactory.getLogger(InputStreamMessageReader::class.java)
    ...
    override fun readUint32(): Long {
        try {
            val u = ByteUtils.readUint32(inputStream)
            cursor += 4
            return u
    ...
    override fun readInt64(): Long {
        try {
            val u = ByteUtils.readInt64(inputStream)
            cursor += 8
            return u
        } catch (e: ArrayIndexOutOfBoundsException) {
            throw ProtocolException(e)
    ...
    @Throws(ProtocolException::class)
    override fun readHash(): Sha256Hash {
        // TODO
