...
import com.nchain.bitcoinkt.utils.BriefLogFormatter;
...
public class BlockChainTest {
    ...
    private static final Coin ZERO = Coin.Companion.getZERO();
    private static final Coin COIN = Coin.Companion.getCOIN();
    private static final Coin FIFTY_COINS = Coin.Companion.getFIFTY_COINS();
    ...
    private CashAddress coinbaseTo;
    ...
    private static final NetworkParameters testNet = TestNet3Params.INSTANCE; // new TestNet2Params(0,10000, 11000, "bchtest");
    ...
    public void setUp() throws Exception {
        BriefLogFormatter.Companion.initVerbose();
        Context.Companion.propagate(new Context(testNet, 100, ZERO, false));
        testNetChain = new BlockChain(testNet, new Wallet(), new MemoryBlockStore(testNet));
        Context.Companion.propagate(new Context(PARAMS, 100, ZERO, false));
        wallet = new Wallet() {
            @Override
...
//        coinbaseTo = wallet.currentReceiveKey().toCashAddress(PARAMS);
    }
    ...
    public void receiveCoins() throws Exception {
        ...
        Transaction tx1 = createFakeTx(PARAMS,
                                       Coin.Companion.getCOIN(),
                                       wallet.currentReceiveKey().toCashAddress(PARAMS));
        Block b1 = createFakeBlock(blockStore, height, tx1).block;
        chain.add(b1);
        assertTrue(wallet.getBalance().getSignum() > 0);
    }

    @Test
    public void unconnectedBlocks() throws Exception {
        Block b1 = GenesisBlock.of(PARAMS).createNextBlock(coinbaseTo);
    ...
    public void difficultyTransitions() throws Exception {
        ...
        Block prev = GenesisBlock.of(PARAMS);
        Utils.INSTANCE.setMockClock(System.currentTimeMillis()/1000);
        for (int height = 0; height < PARAMS.getInterval() - 1; height++) {
            Block newBlock = prev.createNextBlock(coinbaseTo, 1, Utils.INSTANCE.currentTimeSeconds(), height);
            assertTrue(chain.add(newBlock));
            prev = newBlock;
            // The fake chain should seem to be "fast" for the purposes of difficulty calculations.
            Utils.INSTANCE.rollMockClock(2);
        }
        // Now add another block that has no difficulty adjustment, it should be rejected.
        try {
            chain.add(prev.createNextBlock(coinbaseTo, 1, Utils.INSTANCE.currentTimeSeconds(), PARAMS.getInterval()));
            fail();
        } catch (VerificationException e) {
        }
        // Create a new block with the right difficulty target given our blistering speed relative to the huge amount
        // of time it's supposed to take (set in the unit test network parameters).
        Block b = prev.createNextBlock(coinbaseTo, 1, Utils.INSTANCE.currentTimeSeconds(), PARAMS.getInterval() + 1);
    ...
    public void badDifficulty() throws Exception {
        ...
        Block b2 = getBlock2();
        assertTrue(testNetChain.add(b2));
        Block bad = new Block(testNet, Block.Companion.getBLOCK_VERSION_GENESIS());
        // Merkle root can be anything here, doesn't matter.
        bad.setMerkleRoot(Sha256Hash.Companion.wrap("aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"));
        ...
        // solutions.
        bad.setDifficultyTarget(Block.Companion.getEASIEST_DIFFICULTY_TARGET());
        try {
...
*/
    ...
    public void badBip66Version() throws Exception {
        testDeprecatedBlockVersion(Block.Companion.getBLOCK_VERSION_BIP34(), Block.Companion.getBLOCK_VERSION_BIP66());
    }
    */
    /**
    ...
    public void badBip65Version() throws Exception {
        testDeprecatedBlockVersion(Block.Companion.getBLOCK_VERSION_BIP66(), Block.Companion.getBLOCK_VERSION_BIP65());
    }
    ...
    public void intraBlockDependencies() throws Exception {
        ...
        // considered relevant.
        CashAddress somebodyElse = ECKey.Companion.create().toCashAddress(PARAMS);
        Block b1 = GenesisBlock.of(PARAMS).createNextBlock(somebodyElse);
        ECKey key = wallet.freshReceiveKey();
        CashAddress addr = key.toCashAddress(PARAMS);
        // Create a tx that gives us some coins, and another that spends it to someone else in the same block.
        Transaction t1 = FakeTxBuilder.createFakeTx(PARAMS, COIN, addr).build()
        TransactionBuilder t2 = new TransactionBuilder();
        t2.addInput(t1, 0);
        t2.addOutput(Coin.Companion.valueOf(2, 0), somebodyElse);
        b1.addTransaction(t1);
        b1.addTransaction(t2.build());
        b1.solve();
        chain.add(b1);
    ...
    public void coinbaseTransactionAvailability() throws Exception {
        ...
        CashAddress addressToSendTo = receiveKey.toCashAddress(PARAMS);

        // Create a block, sending the coinbase to the coinbaseTo address (which is in the wallet).
        Block b1 = GenesisBlock.of(PARAMS).createNextBlockWithCoinbase(Block.Companion.getBLOCK_VERSION_GENESIS(), wallet.currentReceiveKey().getPubKey(), height++);
        chain.add(b1);
        ...
        assertEquals(ZERO, wallet.getBalance());
        assertEquals(wallet.getBalance(BalanceType.ESTIMATED), Coin.Companion.getFIFTY_COINS());
        assertTrue(!coinbaseTransaction.isMature());

        // Attempt to spend the coinbase - this should fail as the coinbase is not mature yet.
        try {
            wallet.createSend(addressToSendTo, Coin.Companion.valueOf(49, 0));
            fail();
        ...
        for (int i = 0; i < PARAMS.getSpendableCoinbaseDepth() - 2; i++) {
            ...
            Transaction tx2 = createFakeTx(PARAMS, COIN,
                ECKey.Companion.create().toAddress(PARAMS));
        ...
        Transaction tx3 = createFakeTx(PARAMS, COIN, ECKey.Companion.create().toCashAddress(PARAMS));
        ...
        Transaction coinbaseSend2 = wallet.createSend(addressToSendTo, Coin.Companion.valueOf(49, 0));
        ...
        assertEquals(wallet2.getBalance(BalanceType.ESTIMATED), Coin.Companion.valueOf(49, 0));
        assertEquals(wallet2.getBalance(BalanceType.AVAILABLE), Coin.Companion.valueOf(49, 0));
    }

    // block at height 2 on testnet
    private static Block getBlock2() throws Exception {
        Block b2 = new Block(testNet, Block.Companion.getBLOCK_VERSION_GENESIS());
        b2.setMerkleRoot(Sha256Hash.Companion.wrap("20222eb90f5895556926c112bb5aa0df4ab5abc3107e21a6950aec3b2e3541e2"));
        b2.setNonce(875942400);
        b2.setTime(1296688946);
        b2.setDifficultyTarget(0x1d00ffff);
        b2.setPrevBlockHash(Sha256Hash.Companion.wrap("00000000b873e79784647a6c82962c70d228557d24a747ea4d1b8bbe878e1206"));
        assertEquals("000000006c02c8ea6e4ff69651f7fcde348fb9d557a06e6957b65552002a7820", b2.getHashAsString());
        b2.verifyHeader();
        return b2;
    }

    // block at height 1 on testnet
    private static Block getBlock1() throws Exception {
        Block b1 = new Block(testNet, Block.Companion.getBLOCK_VERSION_GENESIS());
        b1.setMerkleRoot(Sha256Hash.Companion.wrap("f0315ffc38709d70ad5647e22048358dd3745f3ce3874223c80a7c92fab0c8ba"));
        b1.setNonce(1924588547);
        b1.setTime(1296688928);
        b1.setDifficultyTarget(0x1d00ffff);
        b1.setPrevBlockHash(Sha256Hash.Companion.wrap("000000000933ea01ad0ee984209779baaec3ced90fa3f408719526f8d77f4943"));
    ...
    public void estimatedBlockTime() throws Exception {
        NetworkParameters params = MainNetParams.INSTANCE;
    ...
    public void falsePositives() throws Exception {
        double decay = AbstractBlockChain.Companion.getFP_ESTIMATOR_ALPHA();
