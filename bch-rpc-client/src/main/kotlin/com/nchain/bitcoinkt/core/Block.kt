 ...
 * [the Bitcoin technical paper](http://www.bitcoinkt.org/bitcoinkt.pdf) for
...
class Block : Message {
    ...
    // TODO: change back to @get:VisibleForTesting and private when tests moved to Kotlin
    @get:VisibleForTesting
    var _transactions: MutableList<Transaction>? = null

    /** If null, it means this object holds only the headers.  */
    val transactions: List<Transaction>?
        get() {
            return if (_transactions == null) null else ImmutableList.copyOf(_transactions)
        }
    ...
    // TODO: change back to @get:VisibleForTesting and private when tests moved to Kotlin
    @get:VisibleForTesting
    var isHeaderBytesValid: Boolean = false

    // TODO: change back to @get:VisibleForTesting and private when tests moved to Kotlin
    @get:VisibleForTesting
    var isTransactionBytesValid: Boolean = false
    ...
    val difficultyTargetAsInteger: BigInteger
        @Throws(VerificationException::class)
        get() {
            val target = ByteUtils.decodeCompactBits(difficultyTarget)
     ...
     * [BIP34: Height in Coinbase](https://github.com/bitcoinkt/bips/blob/master/bip-0034.mediawiki).
     */
    val isBIP34: Boolean
    ...
    /**
     * Returns whether this block conforms to
     * [BIP66: Strict DER signatures](https://github.com/bitcoinkt/bips/blob/master/bip-0066.mediawiki).
     */
    val isBIP66: Boolean
    ...
    /**
     * Returns whether this block conforms to
     * [BIP65: OP_CHECKLOCKTIMEVERIFY](https://github.com/bitcoinkt/bips/blob/master/bip-0065.mediawiki).
     */
    val isBIP65: Boolean
     ...
     */
    @Throws(ProtocolException::class)
    constructor(params: NetworkParameters, payloadBytes: ByteArray, offset: Int, parent: Message?, serializer: MessageSerializer, length: Int) : super(params, payloadBytes, offset, serializer, length) // TODO: Keep the parent
    ...
    constructor(params: NetworkParameters, version: Long, prevBlockHash: Sha256Hash, merkleRoot: Sha256Hash, time: Long,
        ...
        this.nonce = nonce
        this._transactions = LinkedList(transactions)
    }
...
//            optimalEncodingMessageSize += tx.getOptimalEncodingMessageSize()
    ...
    // default for testing
    // TODO: change back to "internal fun" when all tests moved to Kotlin
    ...
    private fun writeTransactions(stream: OutputStream) {
        ...
        if (_transactions != null) {
            stream.write(VarInt(_transactions!!.size.toLong()).encode())
    ...
    override fun bitcoinSerialize(): ByteArray {
        ...
        val stream = UnsafeByteArrayOutputStream(if (length == UNKNOWN_LENGTH) HEADER_SIZE + guessTransactionsLength() else length)
        try {
            writeHeader(stream)
    ...
    private fun guessTransactionsLength(): Int {
        ...
        if (_transactions == null)
            return 0
        var len = VarInt.sizeOf(_transactions!!.size.toLong())
        for (tx in _transactions!!) {
...
//        s.append("   time: ").append(timeSeconds).append(" (").append(Utils.dateTimeFormat(timeSeconds * 1000)).append(")\n")
        ...
        if (_transactions != null && _transactions!!.size > 0) {
            s.append("   with ").append(_transactions!!.size).append(" transaction(s):\n")
            for (tx in _transactions!!) {
                s.append(tx)
            }
        }
    ...
    private fun buildMerkleTree(): List<ByteArray> {
        ...
        var levelSize = _transactions!!.size
        while (levelSize > 1) {
            ...
            while (left < levelSize) {
                ...
                val leftBytes = ByteUtils.reverseBytes(tree[levelOffset + left])
                val rightBytes = ByteUtils.reverseBytes(tree[levelOffset + right])
                tree.add(ByteUtils.reverseBytes(Sha256Hash.hashTwice(leftBytes, 0, 32, rightBytes, 0, 32)))
                left += 2
            }
    ...
    private fun checkTransactions(height: Int, flags: EnumSet<VerifyFlag>) {
        ...
        if (flags.contains(Block.VerifyFlag.HEIGHT_IN_COINBASE) && height >= BLOCK_HEIGHT_GENESIS) {
            // TODO vilches
//            _transactions!![0].checkCoinBaseHeight(height)
        }
        // The rest must not be.
        for (i in 1 until _transactions!!.size) {
    ...
    fun verifyTransactions(height: Int, flags: EnumSet<VerifyFlag>) {
        ...
        if (optimalEncodingMessageSize > MAX_BLOCK_SIZE)
    ...
    // TODO: change to "internal fun" when tests moved to Kotlin
    @VisibleForTesting
    fun setMerkleRoot(value: Sha256Hash) {
        unCacheHeader()
        merkleRoot = value
        hash = null
    }
    ...
    // TODO: change back to "internal fun" when all tests moved to Kotlin
    fun addTransaction(t: Transaction, runSanityChecks: Boolean) {
        unCacheTransactions()
        if (_transactions == null) {
            _transactions = ArrayList()
        }

        ByteUtils
//        t.parent = (this)
        if (runSanityChecks && _transactions!!.size == 0 && !t.isCoinBase)
            throw RuntimeException("Attempted to add a non-coinbase transaction as the first transaction: " + t)
        else if (runSanityChecks && _transactions!!.size > 0 && t.isCoinBase)
            throw RuntimeException("Attempted to add a coinbase transaction when there already is one: " + t)
        _transactions!!.add(t)
        adjustLength(_transactions!!.size, t.length)
    ...
    // TODO: set to "internal fun" when tests moved to Kotlin
    fun setPrevBlockHash(prevBlockHash: Sha256Hash) {
        unCacheHeader()
        this.prevBlockHash = prevBlockHash
        this.hash = null
    }
    ...
    @VisibleForTesting
/*
    fun addCoinbaseTransaction(pubKeyTo: ByteArray, value: Coin, height: Int) {
        unCacheTransactions()
        _transactions = ArrayList()
        val coinbase = TransactionBuilder()
        ...
        coinbase.addInput(TransactionInput(inputBuilder.build().listProgram()))
        coinbase.addOutput(TransactionOutput(value,
                ScriptBuilder.createOutputScript(ECKey.fromPublicOnly(pubKeyTo)).listProgram()))
        val tx:Transaction = coinbase.build()
        _transactions!!.add(tx)

//        coinbase.parent = (this)
//        coinbase.length = coinbase.unsafeBitcoinSerialize().size
        // TODO: vilches revisar
        adjustLength(_transactions!!.size, tx.length)
    }
*/
...
/*
    @VisibleForTesting
    fun createNextBlock(to: CashAddress, version: Long, time: Long, blockHeight: Int): Block {
...
*/
     ...
     */
/*
    internal fun createNextBlock(to: CashAddress?, version: Long,
...
*/
     ...
     */
    // TODO: make this "internal fun" once all tests migrated to Kotlin
/*
//    @VisibleForTesting
    fun createNextBlockWithCoinbase(version: Long, pubKey: ByteArray, height: Int): Block {
        return createNextBlock(null, version, null as TransactionOutPoint?,
...
*/
    ...
    companion object {
         ...
         */
        val MAX_BLOCK_SIZE = 32 * 1000 * 1000
        /**
        ...
        private val pubkeyForTesting = ECKey.create().pubKey
    }
}
