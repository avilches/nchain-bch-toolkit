...
class BloomFilter : Message {
    ...
    @Synchronized operator fun contains(`object`: ByteArray?): Boolean {
        for (i in 0 until hashFuncs) {
            if (!ByteUtils.checkBitLE(data!!, murmurHash3(data!!, nTweak, i.toInt(), `object`!!)))
                return false
        }
        return true
    }
    ...
    fun insert(`object`: ByteArray) {
        for (i in 0 until hashFuncs)
            ByteUtils.setBitLE(data!!, murmurHash3(data!!, nTweak, i.toInt(), `object`))
    }
...
//                        insert(output.outPointFor.unsafeBitcoinSerialize())
        ...
        for (input in tx.inputs) {
            if (contains(input.outpoint!!.bitcoinSerialize())) {
                return true
            }
    ...
    companion object {
        ...
        fun murmurHash3(data: ByteArray, nTweak: Long, hashNum: Int, `object`: ByteArray): Int {
            ...
            val c1:Int = -0x3361d2af
            val c2:Int = 0x1b873593

            val numBlocks = `object`.size / 4 * 4
            // body
            var i = 0
            while (i < numBlocks) {
                var k1:Int = (`object`[i].toInt() and  0xFF) or
