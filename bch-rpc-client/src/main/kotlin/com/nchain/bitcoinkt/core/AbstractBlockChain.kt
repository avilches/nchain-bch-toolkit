...
import com.nchain.bitcoinkt.store.*
...
constructor(context: Context, wallets: List<Wallet>,
     ...
     */
    //*_*CHECK -- this was a protected var -- needed opening up for the getter
     var chainHead: StoredBlock? = null
    /**
    ...
    private val chainHeadLock = ReentrantLock()

    protected val params: NetworkParameters = context.params
    ...
    protected val ruleCheckerFactory = RuleCheckerFactory.create(params)
    ...
    constructor(params: NetworkParameters, transactionReceivedListeners: List<Wallet>,
                _blockStore: BlockStore) : this(Context.getOrCreate(params), transactionReceivedListeners, _blockStore) {
    }
    ...
    private fun add(block: Block, tryConnecting: Boolean,
        ...
        try {
            ...
            if (storedPrev == null) {
                ...
                ruleCheckerFactory.getRuleChecker(storedPrev, block).checkRules(storedPrev, block, blockStore, this)
...
//        return checkpointHash == null || checkpointHash == hash
     ...
     */
    @Throws(BlockStoreException::class, VerificationException::class, PrunedException::class)
    fun handleNewBestChain(storedPrev: StoredBlock, newChainHead: StoredBlock, block: Block?, expensiveChecks: Boolean) {
        checkState(lock.isHeldByCurrentThread)
...
*/
    ...
    // TODO: make "internal fun" when tests are moved to Kotlin
    fun trackFalsePositives(count: Int) {
    ...
    /**
     * has the monolith upgrade been activated
     */
    fun isMonolithEnabled() : Boolean {
        return if ((chainHead?.height ?: 0) < params.monolithHeight) false else true;
    }
