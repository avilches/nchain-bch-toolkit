...
class VersionMessage : Message {
     ...
     */
    var myAddr: PeerAddress? = null
    /**
     * What the other side believes their own address is. Not used.
     */
    var theirAddr: PeerAddress? = null
    /**
     * User-Agent as defined in [BIP 14](https://github.com/bitcoinkt/bips/blob/master/bip-0014.mediawiki).
     * Bitcoin Core sets it to something like "/Satoshi:0.9.1/".
     */
    var subVer: String? = null
    /**
     ...
     * See [BIP 37](https://github.com/bitcoinkt/bips/blob/master/bip-0037.mediawiki#extensions-to-existing-messages).
     */
    ...
    val isGetUTXOsSupported: Boolean
        get() = clientVersion >= GetUTXOsMessage.MIN_PROTOCOL_VERSION &&
                (localServices and NODE_GETUTXOS.toLong()) == NODE_GETUTXOS.toLong()
    ...
    public override fun bitcoinSerializeToStream(buf: OutputStream) {
        ByteUtils.uint32ToByteStreamLE(clientVersion.toLong(), buf)
        ByteUtils.uint32ToByteStreamLE(localServices, buf)
        ByteUtils.uint32ToByteStreamLE(localServices shr 32, buf)
        ByteUtils.uint32ToByteStreamLE(time, buf)
        ByteUtils.uint32ToByteStreamLE(time shr 32, buf)
        try {
            // My address.
    ...
    fun hasBlockChain(): Boolean {
        return (localServices and NODE_NETWORK.toLong()) == NODE_NETWORK.toLong()
    }

    override fun equals(other: Any?): Boolean {
        if (this === other) return true
        if (other == null || javaClass != other.javaClass) return false
        val other = other as VersionMessage?
     ...
     *
     *
     * See [BIP 14](https://github.com/bitcoinkt/bips/blob/master/bip-0014.mediawiki) for more information.
     *
    ...
    companion object {
        ...
        val BITCOINJ_VERSION = "0.0.1"
        /** The value that is prepended to the subVer field of this application.  */
        val LIBRARY_SUBVER = "/bitcoinkt:$BITCOINJ_VERSION/"
