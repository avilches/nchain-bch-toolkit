...
import com.nchain.bitcoinkt.core.listeners.FeeFilterMessage
import com.nchain.bitcoinkt.core.listeners.SendHeadersMessage
 ...
 * [the protocol specification](https://en.bitcoinkt.it/wiki/Protocol_specification).
 *
 *
...
(
    ...
    override fun serialize(name: String, message: ByteArray, out: OutputStream) {
        ...
        ByteUtils.uint32ToByteArrayBE(parameters.packetMagic, header, 0)
        ...
        ByteUtils.uint32ToByteArrayLE(message.size.toLong(), header, 4 + COMMAND_LEN)
...
//            tx.hash = Sha256Hash.wrapReversed(hash)
        ...
        val tx = Transaction.parse(payloadBytes, offset)
        if (hash != null)
            check(Arrays.equals(tx.hash.bytes, hash))
    ...
    constructor(`in`: ByteBuffer) {
        ...
        init {
            ...
            command = ByteUtils.toString(commandBytes, "US-ASCII")
            cursor = COMMAND_LEN

            size = ByteUtils.readUint32(header, cursor).toInt()
            cursor += 4
...
//        private val names = HashMap<Class<out Message>, String>()
        // TODO VILCHES
        private val names = HashMap<Class<out Any>, String>()
