...
class UTXOsMessage : Message {
    ...
    override fun bitcoinSerializeToStream(stream: OutputStream) {
        ByteUtils.uint32ToByteStreamLE(height, stream)
        ...
        for (i in outputs!!.indices) {
            ...
            // TODO VILCHES
            val tx:Transaction? = null // output.parentTransaction
            ByteUtils.uint32ToByteStreamLE(tx?.version ?: 0L, stream)  // Version
            ByteUtils.uint32ToByteStreamLE(heights!![i], stream)  // Height
    ...
    override fun parse() {
        ...
        for (i in 0 until numOuts) {
            ...
            val output = TransactionOutput.parse(payload!!, cursor)
            outputs!!.add(output)
            heights[i] = height
