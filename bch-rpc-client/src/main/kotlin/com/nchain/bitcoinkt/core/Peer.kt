 ...
 * handshake completes.
 */
class Peer  : PeerSocketHandler {
    val blockChain : AbstractBlockChain?
...
constructor(
            ...
            downloadTxDependencyDepth: Int):super(params, remoteAddress){
 ...
 constructor(params: NetworkParameters,
             ...
             remoteAddress: PeerAddress,
             chain: AbstractBlockChain?,
             downloadTxDependencyDepth:Int = Int.MAX_VALUE) : this(params, ver, chain, remoteAddress, downloadTxDependencyDepth)
    /**
     *
    ...
    constructor(params: NetworkParameters,
                ...
                thisSoftwareName: String,
                thisSoftwareVersion: String) : this(params, VersionMessage(params, blockChain.bestChainHeight), peerAddress, blockChain){
...
//            checkState(params.allowEmptyPeerChain() || chainHeight > 0, "Connected to peer with zero/negative chain height", chainHeight)
    ...
    private class GetDataRequest(internal val hash: Sha256Hash, internal val future: SettableFuture<Any>)
...
//*_*CHECK all these casts as (Pong,UTXOsMessage,InventoryMessage,FilteredBlock,Block,AddressMessage,HeadersMessage,AlertMessage,VersionAck,VersionMessage) were nullable
        if (m is Ping) {
            processPing(m)
        } else if (m is Pong) {
            processPong(m)
...
//            processGetData(m)
    ...
    private fun processAddressMessage(m: AddressMessage) {
        ...
        synchronized(getAddrFutures) {
            ...
            future!!.set(m) //*_* CHECK -- this used to be outside the sync
        }
...
//        if (!peerVersionMessage!!.hasBlockChain() || !params.allowEmptyPeerChain() && peerVersionMessage!!.bestHeight == 0L) {
        ...
        if (peerVersionMessage!!.localServices and VersionMessage.NODE_BITCOIN_CASH.toLong() != VersionMessage.NODE_BITCOIN_CASH.toLong()) {
    ...
    protected fun startFilteredBlock(m: FilteredBlock) {
        ...
        if (filteredBlocksReceived % RESEND_BLOOM_FILTER_BLOCK_COUNT == RESEND_BLOOM_FILTER_BLOCK_COUNT - 1) {
            sendMessage(vBloomFilter as Message) //*_*CHECK added cast as Message
        }
    }
    ...
    protected fun processHeaders(m: HeadersMessage) {
        ...
        try {
            ...
            for (i in 0 until m.getBlockHeaders()!!.size) {
                val header = m.getBlockHeaders()!![i]
    ...
    protected fun processTransaction(tx: Transaction) {
        ...
        try {
            ...
            // and so on.
            val confidence = tx.getConfidence()
    ...
    protected fun downloadDependenciesInternal(maxDepth: Int, depth: Int,
        ...
        try {
            ...
            for (hash in needToRequest) {
                ...
                futures.add(req.future as ListenableFuture<Transaction>) //*_*CHECK had to cast to ListenableFuture
...
*/
    ...
    fun getBlock(blockHash: Sha256Hash): ListenableFuture<Block> {
        ...
        return sendSingleGetData(getdata) as ListenableFuture<Block> //*_* added casting
    }
    ...
    fun getPeerMempoolTransaction(hash: Sha256Hash): ListenableFuture<Transaction> {
        ...
        val getdata = GetDataMessage(params)
        getdata.addTransaction(hash)
        return sendSingleGetData(getdata) as ListenableFuture<Transaction> //*_* added casting
    }

    /** Sends a getdata with a single item in it.  */
    private fun sendSingleGetData(getdata: GetDataMessage): ListenableFuture<*> {
        // This does not need to be locked.
        Preconditions.checkArgument(getdata.items!!.size == 1)
        val req = GetDataRequest(getdata.items!![0].hash, SettableFuture.create<Any>())
        getDataFutures.add(req)
        sendMessage(getdata)
        return req.future
    }
    ...
    fun setDownloadParameters(secondsSinceEpoch: Long, useFilteredBlocks: Boolean) {
        ...
        try {
            if (secondsSinceEpoch == 0L) {
                ...
                if (blockChain != null && fastCatchupTimeSecs > blockChain.chainHead!!.header!!.timeSeconds)
    ...
    fun setBloomFilter(filter: BloomFilter?, andQueryMemPool: Boolean) {
        ...
        if(filter == null) {throw IllegalArgumentException("Clearing filters is not currently supported")}
