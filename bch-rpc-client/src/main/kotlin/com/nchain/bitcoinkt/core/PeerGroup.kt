...
open class PeerGroup
     ...
     */
    constructor(context: Context, protected val chain: AbstractBlockChain?, private val channels: ClientConnectionManager = NioClientManager())
    ...
    //*_*CHECK val to var
    protected var executor: ListeningScheduledExecutorService? = null
    ...
    //*_*CHECK -- changed val to var
   @GuardedBy("lock") private var inactives: PriorityQueue<PeerAddress>? = null
   @GuardedBy("lock") private var backoffMap: MutableMap<PeerAddress, ExponentialBackoff>? = null
...
/*
    ...
    private val walletKeyEventListener = object:KeyChainEventListener {
        override fun onKeysAdded(keys: List<ECKey>) {
            recalculateFastCatchupAndFilter(FilterRecalculateMode.SEND_IF_CHANGED)
        }
    }

    private val walletCoinsReceivedEventListener = object: WalletCoinsReceivedEventListener {
...
*/
    ...
    private val triggerConnectionsJob = object : Runnable {
        ...
        fun go() {
            ...
            try {
                ...
                val havePeerWeCanTry = !inactives!!.isEmpty() && backoffMap!![inactives!!.peek()]!!.retryTime <= now
            ...
            try {
                ...
                if (inactives!!.isEmpty()) {
                    ...
                    } while (ipv6Unreachable && addrToTry!!.getAddr() is Inet6Address)
                    retryTime = backoffMap!![addrToTry]!!.retryTime
                }
    ...
    /** See [.PeerGroup]  */
    constructor(params: NetworkParameters, chain: AbstractBlockChain?, connectionManager: ClientConnectionManager) : this(Context.getOrCreate(params), chain, connectionManager) {}
    ...
    init {
        ...
        inactives = PriorityQueue(1, Comparator { a, b ->
            ...
            var result = (backoffMap as HashMap<PeerAddress, ExponentialBackoff>)[a]!!.compareTo((backoffMap as HashMap<PeerAddress, ExponentialBackoff>)[b]!!)
...
*/
     ...
     * [BIP 14](https://github.com/bitcoinkt/bips/blob/master/bip-0014.mediawiki).
     *
    ...
    @VisibleForTesting
    // TODO: make this "internal fun" once all testing moved to Kotlin
    fun waitForJobQueue() {
...
*/
    ...
    fun recalculateFastCatchupAndFilter(mode: FilterRecalculateMode): ListenableFuture<BloomFilter> {
        ...
        synchronized(inFlightRecalculations) {
            if (inFlightRecalculations[mode] != null)
                return inFlightRecalculations!![mode] as ListenableFuture<BloomFilter>
    ...
    protected fun createPeer(address: PeerAddress?, ver: VersionMessage): Peer {
        return Peer(params, ver, chain, address!!,  downloadTxDependencyDepth)
    }
        ...
        try {
            if (downloadPeer != null) {
                if (this.downloadListener != null) {
                    removeDataEventListenerFromPeer(downloadPeer!!, this.downloadListener as PeerDataEventListener)
                }
                if (listener != null) {
    ...
    protected fun handlePeerDeath(peer: Peer, exception: Throwable?) {
        ...
        try {
            ...
            if (exception is NoRouteToHostException) {
                if (address!!.getAddr() is Inet6Address && !ipv6Unreachable) {
    ...
    private inner class ChainDownloadSpeedCalculator : BlocksDownloadedEventListener, Runnable {
        ...
        @Synchronized override fun onBlocksDownloaded(peer: Peer, block: Block, filteredBlock: FilteredBlock?, blocksLeft: Int) {
            ...
            val txCount = (if (blockTransactions != null) countAndMeasureSize(blockTransactions) else 0) + if (filteredBlock != null) countAndMeasureSize(filteredBlock.getAssociatedTransactions().values) else 0
    ...
    fun findPeersWithServiceMask(mask: Int): List<Peer> {
        ...
        try {
            val results = ArrayList<Peer>(peers.size)
            for (peer in peers)
                if (peer.peerVersionMessage!!.localServices and mask.toLong() == mask.toLong())
                    results.add(peer)
            return results
        } finally {
            lock.unlock()
