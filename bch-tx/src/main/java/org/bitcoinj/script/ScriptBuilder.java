...
public class ScriptBuilder {
    ...
    public static Script createOutputScript(CashAddress to) {
        if (to.isP2SHAddress()) {
...
*/
    ...
    public static Script createP2SHOutputScript(Script redeemScript) {
        byte[] hash = ByteUtils.sha256hash160(redeemScript.getProgram());
...
//        Collections.sort(pubkeys, ECKey.PUBKEY_COMPARATOR);
    ...
    public static Script createCLTVPaymentChannelOutput(BigInteger time, ECKey from, ECKey to) {
        byte[] timeBytes = ByteUtils.reverseBytes(ByteUtils.encodeMPI(time, false));
